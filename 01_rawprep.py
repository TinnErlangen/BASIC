...
for sub in subjs:
    ...
    for run in runs:
        ...
        rawmeg.notch_filter(notches,picks=picks,n_jobs=4,notch_widths=breadths) #n_jobs = "cuda": cuda is for use of GPU for processing, if not available on your machine, leave out
        # resample, giving desired sample rate; pass the event list to keep original event time points (!)
        rawmeg,rawmeg_events = rawmeg.resample(200,events=rawmeg_events,n_jobs=4) #n_jobs = "cuda": see above for cuda
